...
def chat_completion(
    data: RouteRequest, 
    myapi_key: str = Header(None), db: Session = Depends(get_db)):
    ...
    verified = myapi.verify_api_key(db=db, api_key_string=api_key)
    if verified:
        ...
        raise HTTPException(
            ...
            detail="Invalid API key. Ensure the correct API key and requesting organization are being used.", 
        )
