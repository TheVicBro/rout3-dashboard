...
from app.schemas import Config, ConfigBase, ConfigBase
...
def edit_configuration(config_update_data: ConfigBase, db: SessionDep, user: UserDep):
    ...
    try:
        ...
        config_model_updated = configurations_repo.update_configuration(
            ...
            route_type=config_model.route_type,
            timeout=config_model.id,
        )
        return config_model_updated
    except Exception as e:
        raise HTTPException(
            status_code=status.HTTP_417_EXPECTATION_FAILED,
            detail="Failed to update configuration record",
        )
...
@router.post("/edit", response_model=ConfigUpdate)
